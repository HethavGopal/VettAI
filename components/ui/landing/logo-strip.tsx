...
const LogoComponents = {
  ...
  Amazon: () => (
    <div className="flex flex-col items-center">
      <div className="font-normal text-xl tracking-wide -mb-1" style={{color: 'currentColor'}}>amazon</div>
      <svg className="w-20 h-3" viewBox="-5 0 100 20" fill="none">
  ...
  Nvidia: () => (
    <div className="font-bold text-xl tracking-wide" style={{color: 'currentColor'}}>NVIDIA</div>
  ),
  Adobe: () => (
    <div className="font-bold text-xl tracking-wider" style={{color: 'currentColor'}}>Adobe</div>
  )
};
  ...
  return (
    <section className="relative py-10 bg-black overflow-hidden border-y border-white/5">
      ...
      <div className="relative">
        {/* Left fade gradient */}
        <div className="absolute left-0 top-0 z-10 h-full w-40 bg-gradient-to-r from-black via-black/90 to-transparent pointer-events-none" />
        
        {/* Right fade gradient */}
        <div className="absolute right-0 top-0 z-10 h-full w-40 bg-gradient-to-l from-black via-black/90 to-transparent pointer-events-none" />
        ...
        >
          {companies.map((company, index) => (
            <div
              ...
              className="flex items-center justify-center mx-16 text-white/40 hover:text-white/80 transition-all duration-500 filter grayscale hover:grayscale-0 cursor-pointer"
            >
              <company.component />
            </div>
          ))}
